...
//  如何发送rst
    ...
    //  You can force the TCPSender to generate an empty segment with the proper sequence number by calling its send empty segment() method. 
    //  Or you can make it fill the window (generating segments if it has outstanding information to send, e.g. bytes from the stream or SYN/FIN) by calling its fill window() method.
...
//  关于State
    //  How do I decipher these “state” names (like “stream started” or “stream ongoing”)?
        ...
        //  but we’re not asking you to materialize these states in your code. You don’t need to make more state variables to keep track of this. 
        //  The “state” is just a function of the public interface your modules are already exposing.
...
//  In an unclean shutdown, the TCPConnection either sends or receives a segment with the rst flag set. 
    //  In this case, the outbound and inbound ByteStreams should both be in the error state, and active() can return false immediately.

//  A clean shutdown is how we get to “done” (active() = false) without an error. 
    //  This is more complicated, but it’s a beautiful thing because it ensures as much as possible that each of the two ByteStreams has been reliably delivered completely to the receiving peer. 
    //  In the next section (§§5.1), we give the practical upshot for when a clean shutdown happens, so feel free to skip ahead if you like
        //  Cool, you’re still here. Because of the Two Generals Problem, it’s impossible to guarantee that both peers can achieve a clean shutdown, but TCP gets pretty close.
        //  Here’s how. From the perspective of one peer (one TCPConnection, which we’ll call the “local” peer), 
        //  there are four prerequisites to having a clean shutdown in its connection with the “remote” peer:
            ...
            // Prereq #2 The outbound stream has been ended by the local application and fully sent (including the fact that it ended, i.e. a segment with fin ) to the remote peer.    (I think)TCPSender :: FIN_SENT
            // Prereq #3 The outbound stream has been fully acknowledged by the remote peer.    (I think)TCPSender :: FIN_ACKED
...
//  Pratical Summary for Clean Shutdown
    //  Practically what all this means is that your TCPConnection has a member variable called linger_after_streams_finish, exposed to the testing apparatus through the state() method. The variable starts out true. 
    ...
    //  At any point where prerequisites #1 through #3 are satisfied, 
        ...
        //  Otherwise you need to linger: the connection is only done after enough time (10 × cfg.rt timeout) has elapsed since the last segment was received
            //  如果linger_after_streams_finish = true , 那么 本连接不结束，而是进入TIME_WAIT
...
//  The peer may choose to send a segment with an invalid sequence number to see if your TCP implementation is still alive (and if so, what your current window is). 
//  Your TCPConnection should reply to these “keep-alives” even though they do not occupy any sequence numbers. Code to implement this can look like this
    ...
    if(_receiver.state() == TCPReceiver::State::FIN_RECV && (_sender.state() == TCPSender::State::SYN_ACKED_2 || _sender.state() == TCPSender::State::SYN_ACKED_1))
    {
        ...
        //  unclean shutdown 之 本端 主动断开连接 . 虽然不用linger , 但也不发送rst报文 , 因为这是正常clean shutdown的一部分。是clean_shutdown中被动关闭的一方
