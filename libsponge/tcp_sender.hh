...
class TCPSender {
  private:
    ...
    // size_t _send_window_size;     //  就是 size_t outstanding_bytes; 就是已经发送
    // 但是未确认的字节数量（占据seq空间）。就是_segment_out payload + SYN + FIN
  ...
  public:
    ...
    //  很遗憾。似乎和我预想的不一样。这个_next_seq
    //  不是ack.也不是在ack_received中更新，而是sender自己维护。发送了seg就自己更新_next_seq，也不用等到该seg被ack，也不用管这个seg到底是否被receiver接收
    enum State {
        ERROR = 0,
        CLOSED,
        SYN_SENT,
